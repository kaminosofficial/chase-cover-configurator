...
import { computeConfiguredPrice } from '../src/utils/pricing.js';
...
// Protected (non-CC-) variant that pins the product's "from $X" to the true
// lowest configurable price. The non-CC- option value means cleanup never
// touches it (cleanup only deletes option1 values starting with "CC-").
...
async function uploadProductImage(productId: string, accessToken: string, base64: string): Promise<{ imageUrl: string; imageId: number }> {
    ...
    const filename = `chase-cover-default-${Date.now()}.${ext}`;
...
/** Lowest configurable price, computed from the LIVE sheet: smallest cover
 *  (16×16×1), no holes, galvanized, cheapest gauge, no powder coat. */
async function computeFloorPrice(): Promise<{ price: string; gauge: number } | null> {
    const pricing: any = await fetchPricingFromPublicSheet(GOOGLE_SHEET_ID, 'pricing');
    if (!pricing || !pricing.GAUGE_MULT) return null;
    const gauges = Object.keys(pricing.GAUGE_MULT)
        .map(Number)
        .filter((n) => Number.isFinite(n) && pricing.GAUGE_MULT[n] > 0);
    if (gauges.length === 0) return null;
    let best = Infinity;
    let bestGauge = gauges[0];
    for (const g of gauges) {
        const total = computeConfiguredPrice(
            { w: 16, l: 16, sk: 1, holes: 0, gauge: g, mat: 'galvanized', pc: false },
            pricing,
            0
        );
        if (total > 0 && total < best) { best = total; bestGauge = g; }
    }
    if (!Number.isFinite(best) || best <= 0) return null;
    return { price: best.toFixed(2), gauge: bestGauge };
}

/** Create the protected anchor variant via REST (non-CC- option value, untracked,
 *  oversell-allowed so it never blocks). Cleanup skips it permanently. */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        if (action === 'upload-product-image') {
            ...
            // instead of a cryptic platform-level rejection.
        ...
        if (action === 'anchor-preview') {
            ...
            return res.status(200).json({
                ...
                gauge: floor.gauge,
                config: { w: 16, l: 16, sk: 1, holes: 0, mat: 'galvanized', gauge: floor.gauge, pc: false },
        ...
        if (action === 'create-anchor') {
            ...
            const existing = variants.find((v: any) => String(v.option1 || '') === ANCHOR_OPTION_LABEL);
            if (existing) {
                // Already present — just keep its price in sync with the current floor.
        ...
        // deleted CC- variant's image_id). No-op if the featured image is already
