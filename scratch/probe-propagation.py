# Measures Shopify variant propagation phases for the chase cover configurator.
# Creates ONE new CC-* variant (auto-cleaned by cron) and times:
#   phase A: POST kaminos.com/cart/add.js until it stops 422ing
#   phase B: GET  kaminos.com/cart.js until our line shows price > 0
#   phase C: GET  product page ?sections=cart-drawer until HTML contains the variant
# Usage: python scratch/probe-propagation.py
import json, time, random, urllib.request, urllib.error, http.cookiejar, sys

API = "https://chase-cover-configurator.vercel.app/api/add-to-cart"
STORE = "https://kaminos.com"
HANDLE = "chase-cover-configurator"
PRODUCT_ID = 10930531696833
...
def req(url, data=None, headers=None, timeout=30):
...
# unique dims so the config hash misses -> forces a brand-new variant
eighth = lambda lo, hi: round(random.randint(lo * 8, hi * 8)) / 8
w, l = eighth(20, 58), eighth(20, 118)
payload = {
    "w": w, "l": l, "sk": 3,
    "drip": True, "diag": True,
    "mat": "galvanized", "gauge": "24",
    "pc": False, "pcCol": "#000000",
    "holes": 0, "quantity": 1, "notes": "",
    "shopifyProductId": str(PRODUCT_ID),
}
print(f"config: w={w} l={l}")

t0 = time.time()
status, body = req(API, payload, timeout=35)
...
# phase C: rendered cart-drawer section contains the variant + no $0
t_sec0 = time.time()
sec_at = None
while time.time() - t_sec0 < 60:
    ...
    is_empty = ("is-empty" in html or "cart is empty" in html.lower())
    if has_variant and not has_zero:
        sec_at = el
        print(f"[D] rendered section has variant + non-zero after {el:.1f}s since add start")
        break
    print(f"    sections: variant={'yes' if has_variant else 'NO'} zero={'yes' if has_zero else 'no'} empty={'yes' if is_empty else 'no'} len={len(html)} at {el:.1f}s")
    time.sleep(1.5)
...
print(f"sections usable   : {sec_at:.1f}s" if sec_at else "sections: NEVER within 60s  <-- drawer would open stale")
total = api_ms/1000 + (sec_at if sec_at else 60)
print(f"==> user-perceived minimum with current flow: ~{api_ms/1000 + (price_at or 0):.1f}s before sections wait, sections add up to +9.5s more")
