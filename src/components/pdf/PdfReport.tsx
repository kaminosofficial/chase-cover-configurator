...
import { holeWorld, getHoleEdgeOffsets } from '../../utils/geometry';
import { RAL_COLORS } from '../../config/ralColors';

// ─────────────────────────────────────────────────────────────────────────────
// PdfReport  —  paginated "Airy" spec-sheet layout
//
// Renders the printable PDF as one or more explicit A4 pages (.pdf-page-render,
// each 794×1123px ≈ 210×297mm @ 96dpi). generatePdf() rasterizes EACH page
...
// once in a hidden measurer, then greedily packed into pages; the pricing card
// is bottom-pinned on the last page (or its own page if it can't fit). The footer
// sits at the bottom of every page (with "Page X of Y" on multi-page exports).
// ─────────────────────────────────────────────────────────────────────────────
...
export function PdfReport({ snapshotUrl }: PdfReportProps) {
  ...
  // The logo sheet is cropped at runtime into a separate symbol + wordmark
  // (getCroppedLogo); falls back to the full logo image until the crop resolves.
  const [logoAssets, setLogoAssets] = useState<{ symbol: string; text: string } | null>(null);
  useEffect(() => {
    getCroppedLogo().then(setLogoAssets);
  }, []);

  // Hero image — big now (~half of page 1). html-to-image / html2canvas don't
  // honor object-fit reliably, so we size it with explicit dims that preserve the
  // image's true aspect ratio, bounded by a max box. Measured on load — never
  // cropped on the sides (it's letterboxed inside the box).
  ...
  // ── Hero render (big, aspect-preserving, never cropped) ──
  ...
  // Edge distances are ALWAYS shown — even for centered holes — derived from the
  // hole's actual world position (matching the on-canvas dimension arrows), so a
  // centered hole still prints its measurements.
  const renderHoleBlock = (id: 'A' | 'B' | 'C', num: number, collar: CollarState) => {
    const sizeStr = collar.shape === 'round'
      ? `${collar.dia}" Dia`
      : `${collar.rectLength}" L x ${collar.rectWidth}" W`;
    const o = getHoleEdgeOffsets(holeWorld(id, config), config);
    const dims = `Top ${formatFrac(o.top)}", Right ${formatFrac(o.right)}", Bottom ${formatFrac(o.bottom)}", Left ${formatFrac(o.left)}"`;
    return (
      <div>
        <SectionLabel>Hole {num} — {collar.shape === 'round' ? 'Round' : 'Rectangle'}</SectionLabel>
        <SpecList>
          ...
          <SpecRow label="Position" value={collar.centered ? 'Centered' : 'Custom Offsets'} />
          <SpecRow label="Edge Distances" value={dims} />
        </SpecList>
      </div>
    );
  };
  ...
  const materialBlock = (
    <div>
      ...
      {config.pc && config.pcCol && (
        ...
        >
          <div style={{ width: '28px', height: '28px', borderRadius: '4px', background: config.pcCol, border: '1px solid rgba(0,0,0,0.12)', flexShrink: 0 }} />
          <div>
            <div style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: C.label }}>Powder Coat Color</div>
            <div style={{ fontSize: '12px', color: '#444', fontWeight: 600 }}>{ralLabel(config.pcCol)}</div>
            <div style={{ fontSize: '10.5px', color: C.muted, marginTop: '1px' }}>{config.pcCol.toUpperCase()}</div>
          </div>
        </div>
      )}
    </div>
  ...
  // Flow blocks, in order. Pricing card is the bottom-pinned trailer.
  const flow: { key: string; node: React.ReactNode }[] = [
    { key: 'hero', node: heroNode(false) },
    { key: 'dimensions', node: dimensionsBlock },
    ...(config.holes >= 1 ? [{ key: 'holeA', node: renderHoleBlock('A', 1, config.collarA) }] : []),
    ...(config.holes >= 2 ? [{ key: 'holeB', node: renderHoleBlock('B', 2, config.collarB) }] : []),
    ...(config.holes === 3 ? [{ key: 'holeC', node: renderHoleBlock('C', 3, config.collarC) }] : []),
    { key: 'material', node: materialBlock },
    ...(notesBlock ? [{ key: 'notes', node: notesBlock }] : []),
  ];
  ...
  // pageNum is only shown on multi-page exports ("Page 1 of 2" etc.).
  const footerNode = (pageNum?: string) => (
    <footer
      style={{
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
        ...
        <span style={{ fontSize: '11.5px', color: '#7A766F', letterSpacing: '0.03em' }}>kaminos.com · 1-888-777-9789</span>
      </div>
    </footer>
  );
  ...
  const [layout, setLayout] = useState<{ pages: number[][]; pricingPage: number } | null>(null);
  ...
  useLayoutEffect(() => {
    ...
    const footerEl = cont.querySelector('[data-measure-footer]') as HTMLElement | null;
    const pricingEl = cont.querySelector('[data-measure-pricing]') as HTMLElement | null;
    if (blockEls.length !== flow.length || !headerEl || !footerEl || !pricingEl) return;

    const headerH = headerEl.offsetHeight;
    const footerH = footerEl.offsetHeight;
    const pricingH = pricingEl.offsetHeight;
    const heights = blockEls.map(el => el.offsetHeight);

    // Every page reserves the footer at the bottom.
    const budget1 = PAGE_H - headerH - PAD_V * 2 - footerH;   // page 1 (after header)
    const budgetN = PAGE_H - PAD_V * 2 - footerH;             // pages ≥ 2
    ...
    // Pricing card bottom-pins on the last page if it fits above the footer;
    // otherwise it gets its own page (still bottom-pinned).
    let pricingPage = pages.length - 1;
    const lastBudget = pricingPage === 0 ? budget1 : budgetN;
    const lastUsed = pages[pricingPage].reduce((s, idx, k) => s + heights[idx] + (k > 0 ? GAP : 0), 0);
    if (lastUsed + GAP + pricingH > lastBudget) {
      pages.push([]);
      pricingPage = pages.length - 1;
    }

    setLayout(prev => {
      const same = prev
        && prev.pricingPage === pricingPage
        && prev.pages.length === pages.length
        && prev.pages.every((p, i) => p.length === pages[i].length && p.every((v, j) => v === pages[i][j]));
      return same ? prev : { pages, pricingPage };
    });
  }, [
    logoAssets, heroDims, snapshotUrl,
    config.w, config.l, config.sk, config.diag, config.drip, config.holes,
    config.collarA, config.collarB, config.collarC,
    config.mat, config.gauge, config.pc, config.pcCol, config.notes,
  ...
  const pricingPage = layout?.pricingPage ?? 0;
  ...
  return (
    <>
      ...
      >
        ...
        <div data-measure-pricing style={{ padding: `0 ${SIDE}px` }}>{pricingNode}</div>
        <div data-measure-footer>{footerNode()}</div>
      </div>

      {/* Paged output — each .pdf-page-render is rasterized into its own A4 page. */}
      <div id="print-mount" style={{ width: `${PAGE_W}px`, background: '#fff', color: C.value, fontFamily: FONT, WebkitFontSmoothing: 'antialiased', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        {pages.map((idxs, pi) => (
          <div
          ...
          >
            ...
            {footerNode(multi ? `Page ${pi + 1} of ${pages.length}` : undefined)}
          </div>
        ))}
      </div>
    </>
