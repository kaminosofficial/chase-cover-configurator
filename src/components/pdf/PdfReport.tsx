...
export function PdfReport({ snapshotUrl }: PdfReportProps) {
  ...
  // hole's real world position (matching the on-canvas dimension arrows), so a
  // centered hole still prints its measurements instead of just "Centered".
  const renderCollarOffsets = (id: 'A' | 'B' | 'C', collar: CollarState) => {
    const o = getHoleEdgeOffsets(holeWorld(id, config), config);
    const dims = `${id} Top: ${formatFrac(o.top)}", ${id} Right: ${formatFrac(o.right)}", ${id} Bottom: ${formatFrac(o.bottom)}", ${id} Left: ${formatFrac(o.left)}"`;
    return collar.centered ? `Centered — ${dims}` : dims;
  };
...
function SnapshotBlock({ snapshotUrl, heroDims, onHeroLoad }: { snapshotUrl?: string; heroDims: any; onHeroLoad: any }) {
  return (
    <div style={{ flexShrink: 0, padding: '8px 53px 8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      {snapshotUrl ? (
        <img
          src={snapshotUrl}
          ...
          style={
            heroDims
              ...
              : { maxWidth: '640px', maxHeight: '350px', width: 'auto', height: 'auto', display: 'block' }
          }
        />
      ) : (
