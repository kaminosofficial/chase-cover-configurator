...
const CART_FAILURE_STORE_KEY = 'chase-cart-failures';
...
function summarizeFailedConfig(config: any): Record<string, any> | null {
  ...
  return {
    ...
    holes: config.holes, pc: config.pc, qty: config.quantity,
  };
}
...
async function flushCartFailureReports(apiBase: string): Promise<void> {
  ...
  try {
    ...
    if (data && data.persisted) {
      // Drop the records we just persisted; keep any added while in flight.
 ...
 * properties — properties matter: another configured cover in the cart keeps
 * its _config_json) to sessionStorage before clearing, and re-add whatever is
 * missing when the customer comes back. Per-tab, one-shot, time-boxed; every
 * failure path degrades to today's behavior (cart stays cleared). Re-adding
 * after a COMPLETED order is correct too — that matches what a native Buy Now
 * would have left in the cart. */
const CART_SNAPSHOT_KEY = 'chase-cover-cart-snapshot';
...
async function captureCanvasScreenshot(
  ...
  const previousLabels = shouldHideLabels
    ...
    : null;
  // Move-holes mode draws orange drag rings + a "Drag to Move" badge over each
  // hole — never wanted in a snapshot/PDF. Hide for the capture, restore after.
  const wasMoveHolesMode = configStore.moveHolesMode;

  try {
    if (wasMoveHolesMode) {
      configStore.set({ moveHolesMode: false });
...
export default function App({ productId, variantId }: AppProps = {}) {
  ...
  useEffect(() => {
    if (!(pdfModalOpen || ralOpen)) return;
    ...
    <>
      ...
      >
        ...
        <Sidebar
          ...
          onExportPdf={() => {
            // Exit move-holes mode first: otherwise the orange drag rings show
            // in the PDF snapshot and the high-z-index "Drag to Move" badge
            // floats over the preview modal (same reason Add to Cart / Buy Now
            // exit it before capturing).
            if (config.moveHolesMode) setConfig({ moveHolesMode: false });
        ...
        <Suspense fallback={null}>
          <PdfPreviewModal
            ...
            captureSnapshot={async () => {
              return captureCanvasScreenshot(appLayoutRef, { resetView: true, hideLabels: true, framed: true });
            }}
          />
        </Suspense>
      )}
