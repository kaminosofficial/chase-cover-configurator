...
1. **Snapshots the existing cart lines first** (`saveCartSnapshotBeforeClear` → sessionStorage `chase-cover-cart-snapshot`: variant id, quantity, line properties; 60-min expiry)
2. Calls `/cart/clear.js` (Shopify's `/checkout` checks out the WHOLE cart, so it must contain only the configured item — note this is NOT how native dynamic-checkout buttons work; they never touch the cart)
3. 300ms pause after clear to avoid 429
4. No sections needed (redirects to `/checkout` instead of opening drawer)
5. Image upload is `await`ed before redirect (prevents mobile browser from cancelling in-flight request)

**Cart restore (June 2026)**: when the customer returns (back-button/bfcache via the `pageshow` handler, or full-reload via the mount path), `restoreCartSnapshotIfNeeded()` re-adds any snapshot lines missing from the cart — batch `/cart/add.js` first, per-item fallback if the batch fails (one sold-out line must not block the rest), then `syncCartUiFromStorefront('CART-RESTORE')` refreshes badge + drawer. One-shot key (deleted before re-adding; re-saved only if NOTHING restored, e.g. a throttle window). Restoring after a COMPLETED order is intentional — native Buy Now leaves those items in the cart too. Per-tab sessionStorage: no server state, no cross-user interaction; every failure path degrades to the old behavior (cart stays cleared).
...
- **Drawer staleness + latency fixes (June 12, 2026 — chase only, cap port pending user test):**
  ...
  - **Pre-open section ceiling raised to 30s (June 13, 2026, evening)**: the drawer must open WITH the item already rendered. For a brand-new variant, Shopify's server-rendered cart-drawer shows the line at $0 until the variant price propagates to the render layer — measured live up to ~26s. An 8s, then 12s, ceiling still flashed empty on slow new-variant adds (the catch-up filled it ~2s later, which the client found confusing). Now `maxWaitMs` for the `CART-SECTIONS` wait is 30000ms; the `CartProgressOverlay` "Opening your cart" step + a reassuring note ("Almost there — finalizing your cart preview") cover the wait. Safe because the throttle-bail (2 throttled fetches → `rateLimited`) means the big budget is only ever spent on genuine propagation lag, which resolves; a blocked IP still bails in ~3s. The post-open catch-up remains the safety net for the rare >30s tail. PDF/drag-to-move snapshot fix shipped the same day (see "Drag-to-move PDF" below).
  - **Mobile sticky-viewer z-index lowered 100 → 1 (June 13, 2026)**: on mobile the `.viewport` goes `position: sticky; top: 0` (JS observer in `shopify-entry.tsx`) so the 3D viewer pins while the sidebar scrolls. It carried `z-index: 100`, which let the pinned viewer paint OVER the Shopify theme's open search dropdown (reported on the cap; the code is byte-identical to chase, so chase was equally vulnerable — only theme/section placement made it surface on the cap). The theme header is `z-index: 1000`; the sidebar is non-positioned, so `z-index: 1` keeps the pinned viewer above the sidebar but safely below all theme chrome. Lowering it doesn't affect the viewer's internal layering (canvas `z-index:1` / controls `z-index:5` live in the viewport's own stacking context). NOTE: this is a theme-integration issue — it cannot be reproduced in the standalone dev preview (no theme header there); verify on the live Shopify embed.
  - **Modal scroll-lock + PDF color name (June 13, 2026)**: the PDF preview and RAL color-picker modals no longer let the store page scroll behind them — `overscroll-behavior: contain` on their scroll containers (PDF body div + `.ral-grid-container`/`.ral-modal-overlay`) stops chaining, and an App.tsx effect locks `document` overflow while `pdfModalOpen || ralOpen` (restored on close; scoped to these two modals, the cart overlay manages its own). The PDF's Powder Coat Color now shows the RAL name + code ("Ruby Red (RAL 3002)") via `ralLabel()` (reuses `RAL_COLORS` from `config/ralColors`), with the hex on a small line below; falls back to hex for non-RAL colors.
  - **Drag-to-move PDF snapshot (June 13, 2026)**: `onExportPdf` now exits `moveHolesMode` before opening the preview (mirrors Add to Cart / Buy Now), and `captureCanvasScreenshot` hides `moveHolesMode` during the capture. Without this, the orange drag rings showed in the PDF snapshot and the drei `<Html>` "Drag to Move" badge (very high z-index) floated over the preview modal. Move mode is intentionally NOT restored after capture (matches cart/buy, prevents the badge reappearing over the open modal). Cap has no drag-to-move feature, so this is chase-only.
  ...
  - **Premature-open guard (June 13, 2026)**: while an add/buy is in flight, `startPrematureCartOpenGuard` (350ms interval) force-closes any cart drawer that opens before the flow's own open step, releasing stray scroll locks. Root cause: the theme header (and its cart icon, bound by the theme's `cart-drawer.js` to open the drawer) stays clickable above the progress overlay, and the submission-time z-chain raise painted the configurator OVER an early-opened drawer — user saw a half-buried drawer (header + SUBTOTAL visible, blurred configurator in the middle) with stale contents and the default product image, observed live with a slow-propagating copper variant. A drawer open during the pre-open section injection also re-entered the innerHTML-on-open-drawer failure mode — that site now falls back to `applySectionUpdatesPreservingCartUi` if the drawer is somehow open. Guard stops right before `tryOpenCartUi()` (cart), before the checkout redirect (buy), and in both `finally` blocks.
...
- **429 rate limiting**: Shopify storefront 429s are retried with a longer backoff (~3.5–4.5s) inside `addToCartWithRetry` / `continueCartPreparationUntilVerified` — but a SUSTAINED throttle window exits early (3 hits in the main loop / 2 in the continuation, ~15–20s) with `rateLimited: true` and the friendly "store is busy" message, instead of grinding the full ~45s budget and throwing the misleading generic "could not confirm / refresh" error (which is what a rapid Buy Now test produced on June 12).
- **Throttle-aware fetches (June 13, 2026)**: bot protection answers **429 or 430** (`isThrottleStatus`). `fetchRenderedSections` and `fetchCartState` now return `{sections|cart, status}` (was bare value / null) so callers can tell a rate-limited fetch from a genuinely-not-ready one — a 429 challenge page is HTML, which the old code silently nulled. Consequences:
  - `waitForUsableRenderedSections` bails after **2 consecutive** throttle statuses (returns `rateLimited: true`) instead of burning the 12s ceiling on doomed, score-feeding polls.
  - The pre-open branch: if sections never became usable AND `rateLimited`, the drawer is **not** opened (the catch-up loop would be just as blocked) — instead an honest alert says the item was added and to use the cart icon/page. Non-throttled stale → open + catch-up as before.
  - Read-throttle guard: after the add POST returns 200, **3 consecutive throttled `fetchCartState` reads** exit with "your item may already be in the cart" — prevents the old failure where blocked reads ground the full budget into a generic "try again" that invited a **duplicate add** (likely the June 12 failed Buy Now).
  - `startPostOpenDrawerCatchUp` stretches its interval to 5s when throttled.
  - `emitCartDebug('storefront-rate-limited', …)` fires once per flow (server-side evidence for the next incident).
  - Request-footprint trims (lower bot score): price-wait poll 1500→2000ms, confirm 1200→1500ms, section `delayMs` 1200→1500ms, restore per-item fallback spaced 250ms, and the redundant per-add `verifiedSections` pre-fetch deleted.
  - **None of this clears an already-flagged IP** — that's Shopify-side. A top-level visit to the cart page (which runs the interactive challenge JS that background `fetch()` can't), IP quiet time, or a different network are the only remedies; the message now says so. (The June 2026 unified-loop rewrite had lost this — only 422 was retried — so Shopify's raw bot-challenge text leaked into the error alert during a stress test; restored June 13, 2026.) HTML challenge-page bodies are never used as error messages, and `formatCheckoutErrorMessage` maps any "too many/rate limit/throttle/429" text to a friendly "store is busy" message. Shopify's per-IP limit was reproduced live at ~46 rapid adds in 40s; mobile carrier CGNAT (shared IPs) lowers the effective threshold.
...
- **`/api/add-to-cart` network resilience (June 13, 2026, evening)**: `postAddToCartApi` retries **5×** with a `600ms→3.5s` backoff (`getAddToCartApiRetryDelayMs`), spanning ~12s of wall-clock for fast-rejecting network errors. Raised from 3 attempts / ~3s after a user hit "network issue" on a **fresh device on mobile data** — verified server-side healthy (direct POST of the exact config returned 200 with correct price; runtime logs showed only clean invocations), so the failure was a transient mobile-data drop (tower handoff / weak signal) longer than the old 3s give-up window; no variant was created. Timeouts are still capped at 2 attempts (`timeoutRetryExhausted`), so only fast network rejects use all 5 — a stuck cold start can't run 5×30s. A genuine full connection drop on the client still can't be fixed in code (the honest "network issue — check your connection" message remains).
